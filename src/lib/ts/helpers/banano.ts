...
function getAddressFromPublic(accountPublicKeyBytes: any, prefix = 'ban') {
	...
	const checksum = uint5ToString(uint4ToUint5(uint8ToUint4(blake2bFinal(context).reverse())));
...
function hexToUint4(hexValue: any) {
...
function uint8ToUint4(uintValue: any) {
...
function uint8ToHex(uintValue: any) {
	let hex = '';
	let aux;
	for (let i = 0; i < uintValue.length; i++) {
...
function uint4ToUint8(uintValue: any) {
...
function uint4ToUint5(uintValue: any) {
...
function uint5ToString(uint5: any) {
...
function decodeNanoBase32(input: any) {
