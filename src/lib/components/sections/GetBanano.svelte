...
<div id="get-banano" class="w-full py-8">
	...
	>
		<div class="container-b flex flex-col items-center px-4">
			...
			<p class="mt-3 text-xl container-b-small md:px-8">
				First, be sure to join our Discord, Reddit & Telegram. Then you can participate in random
				airdrops, monkey around with random tips/rains.<br />We also do giveaways on Twitter, real
				ones. No need to send us any Ethereum.
			</p>
			...
			<p class="mt-3 text-xl container-b-smallest md:px-8">
			...
			<div class="w-full flex flex-wrap justify-center items-end mt-2 md:mt-4 md:px-8">
				{#each faucets as faucet}
					{#if !faucet.isStopped}
						...
						>
							<h3 class="font-bold text-lg">{faucet.title.toUpperCase()}</h3>
							<p class="mt-1 px-2">{faucet.description}</p>
							<Button class="mt-4 w-full" href={faucet.buttonUrl} targetBlank={true}
